...
// and MCP surfaces cannot describe different behaviour. That module also carries the AST
// fence around mathjs — this endpoint is public and compiles caller-supplied source.
  ...
  switch (method) {
    case 'initialize':
      return reply({
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'curvely', version: VERSION },
      });
    ...
    case 'tools/call': {
      ...
      try {
        ...
        if (err instanceof ToolError) {
          return reply({ content: [{ type: 'text', text: `curvely: ${err.message}` }], isError: true });
        }
        throw err;
      }
      if (!result) {
